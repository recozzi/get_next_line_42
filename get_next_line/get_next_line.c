...
/*   Updated: 2023/01/09 17:18:14 by recozzi          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	if (fd < 0 || BUFFER_SIZE <= 0)
		return (0);
	if (!ft_read_and_check(fd, &keeper) || !keeper || !*keeper)
		return (ft_free((void **)&keeper));
	line = ft_get_a_line(keeper);
	ft_save_extra_char(&keeper, line);
	return (line);
}
